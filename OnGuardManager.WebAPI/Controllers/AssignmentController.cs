...
namespace OnGuardManager.WebAPI.Controllers
{
	...
	public class AssignmentController : ControllerBase
	{
		...
		public async Task<IActionResult> GetAllCommonUnity()
		{
			...
			List<User> users = await _userService.GetAllUsersByCenterRules(1);

			foreach (User user in users)
			{
				userStats.Add(new UserStats(user, 0, 0, 0));
			}


			string properties = "totalGuardMonth";
			ExpressionType operations = ExpressionType.LessThan;
			string value = "6";


			List<UserStats> assignmentFiltered = FilterList<UserStats>.FilterQuerySimple(userStats.AsQueryable(), properties,
																					operations, value).ToList();
			
			string properties2 = "user.IdLevelNavigation.Name";
			ExpressionType operations2 = ExpressionType.Equal;
			string value2 = "R5";

			List<UserStats> assignmentFiltered2 = FilterList<UserStats>.FilterQueryMultipleProperty(userStats.AsQueryable(), properties2,
																					operations2, value2).ToList();

			return Ok(true);
			/*List<UserModel> users = await _userService.GetAllUsersByCenter(1);
